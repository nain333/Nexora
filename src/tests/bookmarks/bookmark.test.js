...
describe("Bookmarks API", () => {
  ...
  beforeAll(async () => {
    ...
    expect(
      postsResponse.body.data.posts.length,
    ).toBeGreaterThan(0);
  ...
  describe("Get Bookmarks", () => {
    it("BOOKMARK-05 should retrieve the authenticated user's bookmarks", async () => {
      ...
      expect(
        Array.isArray(response.body.data.bookmarks),
      ).toBe(true);
    });
  ...
  describe("Create Bookmark", () => {
    it("BOOKMARK-07 should create a bookmark successfully", async () => {
      ...
      expect(
        response.body.data.bookmark,
      ).toBeDefined();

      expect(
        response.body.data.bookmark.postId,
      ).toBe(postId);

      expect(
        response.body.data.bookmark.id,
      ).toBeDefined();

      expect(
        response.body.data.bookmark.userId,
      ).toBeDefined();

      expect(
        response.body.data.bookmark.createdAt,
      ).toBeDefined();
    });
    ...
    it("BOOKMARK-08 should expose the newly created bookmark through GET bookmarks", async () => {
      ...
      const bookmark =
        response.body.data.bookmarks.find(
    ...
    it("BOOKMARK-10 should reject an invalid post UUID", async () => {
      const response = await request(app)
        .post(
          "/api/bookmarks/not-a-valid-uuid",
        )
        .set(
          "Authorization",
          `Bearer ${primaryToken}`,
    ...
    it("BOOKMARK-11 should return 404 when bookmarking a nonexistent post", async () => {
      const response = await request(app)
        .post(
          "/api/bookmarks/00000000-0000-4000-8000-000000000000",
        )
        .set(
          "Authorization",
          `Bearer ${primaryToken}`,
  ...
  describe("User Isolation", () => {
    it("BOOKMARK-13 should return only the Primary User's bookmarks", async () => {
      ...
      const bookmarks =
        response.body.data.bookmarks;

      expect(
        bookmarks.every(
          (bookmark) =>
            bookmark.userId === primaryUserId,
        ),
      ).toBe(true);
    });
    ...
    it("BOOKMARK-14 should return only the Secondary User's bookmarks", async () => {
      ...
      expect(
        bookmarks.every(
          (bookmark) =>
            bookmark.userId === secondaryUserId,
        ),
      ).toBe(true);
    });
  });
