...
describe("Posts API", () => {
  ...
  beforeAll(async () => {
    ...
    primaryToken =
      primaryLoginResponse.body.data.accessToken;
    ...
    expect(secondaryLoginResponse.body.status).toBe(
      "success",
    );

    secondaryToken =
      secondaryLoginResponse.body.data.accessToken;
    ...
    primaryPostId =
      primaryPostResponse.body.data.post.id;

    const secondaryPostResponse = await request(app)
      .post("/api/posts")
      .set(
        "Authorization",
        `Bearer ${secondaryToken}`,
      )
      .send({
    ...
    secondaryPostId =
      secondaryPostResponse.body.data.post.id;
  });
  ...
  describe("Authentication", () => {
    ...
    it("POST-07 should reject status updates without authentication", async () => {
      const response = await request(app)
        .patch(
          `/api/posts/${primaryPostId}/status`,
        )
        .send({
          status: "draft",
        });
  ...
  describe("Create Post", () => {
    it("POST-09 should create a post successfully", async () => {
      ...
      expect(response.body.data.post.userId).toBe(
        primaryUserId,
      );
      expect(response.body.data.post.caption).toBe(
        "Automated created post",
      );
      expect(response.body.data.post.status).toBe(
        "published",
      );
      expect(
        response.body.data.post.createdAt,
      ).toBeDefined();
    });
    ...
    it("POST-15 should accept a caption exactly 2200 characters long", async () => {
      ...
      expect(response.body.status).toBe("success");
      expect(
        response.body.data.post.caption.length,
      ).toBe(2200);
    });
  ...
  describe("Get User Posts", () => {
    it("POST-17 should retrieve the authenticated user's posts", async () => {
      ...
      expect(response.body.status).toBe("success");
      expect(
        Array.isArray(response.body.data.posts),
      ).toBe(true);
    });
    ...
    it("POST-18 should return only posts belonging to the authenticated user", async () => {
      ...
      expect(
        response.body.data.posts.every(
          (post) => post.userId === primaryUserId,
        ),
      ).toBe(true);
    });
  });
  ...
  describe("Get Post By ID", () => {
    it("POST-19 should retrieve an existing post", async () => {
      ...
      expect(response.statusCode).toBe(200);
      expect(response.body.status).toBe("success");
      expect(response.body.data.post.id).toBe(
        primaryPostId,
      );
    });
  ...
  describe("Post Status", () => {
    ...
    it("POST-38 should reject an invalid UUID for status updates", async () => {
      const response = await request(app)
        .patch(
          "/api/posts/not-a-valid-uuid/status",
        )
        .set(
          "Authorization",
          `Bearer ${primaryToken}`,
    ...
    it("POST-39 should return 404 for status updates on a nonexistent post", async () => {
      const response = await request(app)
        .patch(
          `/api/posts/${nonexistentUUID}/status`,
        )
        .set(
          "Authorization",
          `Bearer ${primaryToken}`,
  ...
  describe("Feed Filtering and Pagination", () => {
    ...
    it("POST-41 should filter posts by caption case-insensitively", async () => {
      const response = await request(app)
        .get(
          "/api/posts/all?caption=post%20test%20secondary",
        )
        .set(
          "Authorization",
          `Bearer ${primaryToken}`,
      ...
      expect(response.statusCode).toBe(200);
      expect(
        response.body.data.posts.length,
      ).toBeGreaterThan(0);
    ...
    it("POST-42 should return an empty collection for an unmatched caption", async () => {
      const response = await request(app)
        .get(
          "/api/posts/all?caption=NO_MATCH_UNIQUE_938274",
        )
        .set(
          "Authorization",
          `Bearer ${primaryToken}`,
      ...
      expect(
        response.body.data.pagination.totalPosts,
      ).toBe(0);
    });
    ...
    it("POST-43 should apply pagination correctly", async () => {
      ...
      expect(response.statusCode).toBe(200);
      expect(response.body.data.posts.length).toBeLessThanOrEqual(
        2,
      );
      expect(
    ...
    it("POST-44 should return numeric pagination metadata", async () => {
      ...
      expect(response.statusCode).toBe(200);
      expect(
        typeof response.body.data.pagination.currentPage,
      ).toBe("number");
      expect(
        typeof response.body.data.pagination.limit,
      ).toBe("number");
      expect(
        typeof response.body.data.pagination.totalPosts,
      ).toBe("number");
      expect(
    ...
    it("POST-51 should reject unsupported query parameters", async () => {
      const response = await request(app)
        .get(
          "/api/posts/all?page=1&limit=2&role=admin",
        )
        .set(
          "Authorization",
          `Bearer ${primaryToken}`,
    ...
    it("POST-52 should combine caption filtering and pagination correctly", async () => {
      const response = await request(app)
        .get(
          "/api/posts/all?caption=POST%20TEST&page=1&limit=1",
        )
        .set(
          "Authorization",
          `Bearer ${primaryToken}`,
  ...
  describe("Date Sorting", () => {
    it("POST-53 should sort the default feed newest-first", async () => {
      ...
      for (let index = 1; index < posts.length; index++) {
        ...
        expect(previousDate).toBeGreaterThanOrEqual(
          currentDate,
        );
      }
    });
    ...
    it("POST-54 should explicitly sort by date newest-first", async () => {
      ...
      for (let index = 1; index < posts.length; index++) {
        expect(
          ...
          new Date(posts[index].createdAt).getTime(),
        );
      }
    });
  ...
  describe("Engagement Sorting", () => {
    ...
    it("POST-56 should create controlled posts with different engagement levels", async () => {
      ...
      mediumEngagementPostId =
        mediumResponse.body.data.post.id;
      ...
      highEngagementPostId =
        highResponse.body.data.post.id;

      const mediumLikeResponse = await request(app)
        .get(
          `/api/likes/toggle/${mediumEngagementPostId}`,
        )
        .set(
          "Authorization",
          `Bearer ${primaryToken}`,
      ...
      const highPrimaryLikeResponse = await request(app)
        .get(
          `/api/likes/toggle/${highEngagementPostId}`,
        )
        .set(
          "Authorization",
          `Bearer ${primaryToken}`,
      ...
      const highCommentResponse = await request(app)
        .post(
          `/api/comments/${highEngagementPostId}`,
        )
        .set(
          "Authorization",
          `Bearer ${primaryToken}`,
    ...
    it("POST-57 should sort controlled posts by engagement descending", async () => {
      ...
      const ids = response.body.data.posts.map(
        (post) => post.id,
      );
    ...
    it("POST-58 should sort by engagement before applying pagination", async () => {
      ...
      expect(
        firstPageResponse.body.data.posts.length,
      ).toBe(1);
      expect(
        firstPageResponse.body.data.posts[0].id,
      ).toBe(highEngagementPostId);
      ...
      expect(
        secondPageResponse.body.data.posts[0].id,
      ).toBe(mediumEngagementPostId);
      ...
      expect(
        thirdPageResponse.body.data.posts[0].id,
      ).toBe(lowEngagementPostId);
    });
  });
